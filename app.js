(function () {
  ...
  function renderCoverage() {
    var html =
      '<ul>' +
      '<li><strong>ACJS (careers.acjs.org)</strong> — both public pages of the careers board collected on July 20, 2026. After applying the criminology / criminal-justice relevance filter, 8 specific postings retained; the rest excluded as adjunct-pool catch-alls, EMT / paramedic / paralegal, generic auxiliary-services roles, or unrelated counseling postings. Posted dates were given as relative ("X days ago") and converted to absolute dates relative to capture date.</li>' +
      '<li><strong>ASC (asc41.org)</strong> — single static page, no pagination, captured July 20, 2026. All 12 publicly listed postings captured.</li>' +
      '<li><strong>HigherEdJobs (higheredjobs.com)</strong> — Criminal Justice & Criminology faculty category, first 50 active postings captured (July 20, 2026). 17 retained as faculty + research + practitioner roles (assistant/associate professors, lecturers, instructors, visiting faculty, teaching faculty, postdoctoral scholars, professor of the practice); the rest excluded as generic adjunct pools, dual-credit, police-academy training, pure forensic-science, or graduate-assistant roles.</li>' +
      '<li><strong>jobs.ac.uk</strong> — the keyword=criminology search captured July 20, 2026; 9 retained as clearly criminology-relevant (including criminology-adjacent sociology, policing, and law posts housed in criminology departments). The remainder excluded as pure law-school lectureships, generic methodology fellowships, or unrelated technician/editor roles.</li>' +
      '<li><strong>TSPA (tspa.org)</strong> — the public Trust &amp; Safety job board, first indexed May 12, 2026 and last refreshed July 6, 2026 (TSPA refreshes monthly, on the first Monday of each month; carried over verbatim on weekly-only runs). 67 publicly listed postings included — TSPA covers industry trust-and-safety, policy, and content-moderation roles at tech companies (e.g. Google, Discord, Meta, TikTok). No filtering applied: roles span analyst, specialist, manager, lead, engineer, and director levels. Deadlines are not exposed on the source listing, so that column is blank for every TSPA row. <strong>Posted dates</strong> are also not exposed by TSPA; as a substitute, each TSPA row is stamped with the date this dashboard first indexed it, so "Newest posted" sort approximates "most recently seen" rather than the true publication date.</li>' +
      ...
      '<li><strong>De-duplication</strong> — records are matched across sources by normalized institution + normalized title; matches are merged into a single row with a comma-separated source_site (e.g. "ASC, HigherEdJobs") and all source URLs concatenated in combined_urls. The current snapshot has 9 cross-site duplicates merged into single rows.</li>' +
      "</ul>";
